...
export default async function AdminMessagesPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold">{t("admin.messages_title")}</h1>
        <p className="text-cocoa-500">{t("admin.messages_desc")}</p>
      </div>

      <div className="space-y-4">
        {messages.length === 0 ? (
          <div className="p-8 text-center text-cocoa-400 bg-white rounded-xl border border-cocoa-100">{t("admin.no_messages")}</div>
        ) : (
          messages.map(msg => (
            <div key={msg.id} className={`bg-white rounded-xl shadow-sm border p-6 ${msg.isRead ? 'border-cocoa-50 opacity-70' : 'border-accent border-l-4'}`}>
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-sm text-cocoa-500">{t("admin.from")} <span className="font-semibold text-primary">{msg.name}</span> ({msg.email})</p>
                </div>
