    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-cocoa-50 text-primary border-b border-cocoa-100 uppercase text-xs tracking-wider">
                ...
                <th className="p-4">{t("admin.col_products_linked")}</th>
            ...
            <tbody className="divide-y divide-cocoa-50">
              {categories.map(cat => (
                <tr key={cat.id} className="hover:bg-amber-50/20">
                  ...
                  <td className="p-4 font-bold text-cocoa-500">{cat._count.products} {t("admin.items_suffix")}</td>
      ...
      <div>
        <div className="bg-primary text-beige rounded-xl p-6 shadow-xl sticky top-24">
           <h2 className="text-xl font-bold mb-4 text-white">{t("admin.add_category")}</h2>
           <form action={addCategory} className="space-y-4">
             <div>
               <label className="block text-sm font-medium mb-1">{t("admin.category_name")}</label>
