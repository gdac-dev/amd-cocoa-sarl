  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-cocoa-500">{t("admin.users_desc")}</p>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-cocoa-50 text-primary border-b border-cocoa-100 uppercase text-xs tracking-wider">
              <th className="p-4">{t("admin.col_name")}</th>
              <th className="p-4">{t("admin.col_email")}</th>
              <th className="p-4">{t("admin.col_role")}</th>
              <th className="p-4">{t("admin.col_verified")}</th>
              <th className="p-4">{t("admin.col_joined")}</th>
              ...
              <tr key={user.id} className="hover:bg-amber-50/20">
                ...
                <td className="p-4">
                  ...
                  {user.isSeller && (
                    <span className={`ml-1 px-2 py-1 rounded text-xs font-bold ${user.isApproved ? 'bg-amber-100 text-amber-700' : 'bg-orange-100 text-orange-700'}`}>
                      {user.isApproved ? t("admin.seller_badge") : t("admin.seller_pending")}
                    </span>
                  )}
                </td>
                <td className="p-4 flex gap-1">
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${user.isEmailVerified ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                    {user.isEmailVerified ? t("admin.verified_yes") : t("admin.verified_no")}
                  </span>
                  {!user.isActive && (
                    <span className="px-2 py-1 rounded-full text-xs font-semibold bg-gray-200 text-gray-700">{t("admin.deactivated")}</span>
                  )}
                </td>
                       ...
                       <>
                         {user.isSeller && !user.isApproved && (
                           <form action={approveSeller}>
                             ...
                             <button type="submit" className="text-green-600 hover:underline text-sm font-bold bg-green-100 px-2 py-1 rounded">{t("admin.approve_seller")}</button>
                           </form>
                         )}
                         ...
                         <form action={deleteUser}>
                           <input type="hidden" name="id" value={user.id} />
                           <button type="submit" className="text-red-500 hover:underline text-sm font-bold">{t("admin.delete")}</button>
                         </form>
                       </>
                     )}
                   </div>
